...
public class DatabaseManager {
    ...
    public static DatabaseManager createDefault() {
        ...
        ensureLocalDataDirectoryExists(jdbcUrl);
    ...
    private static void ensureLocalDataDirectoryExists(String jdbcUrl) {
        if (jdbcUrl.startsWith("jdbc:h2:file:./data/")) {
            try {
                Files.createDirectories(Path.of("data"));
            } catch (Exception ex) {
                throw new IllegalStateException("Could not create local data directory for H2 database.", ex);
            }
        }
    }
